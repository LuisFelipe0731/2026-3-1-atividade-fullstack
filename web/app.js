...
function renderProfile() {
  ...
  const ownPosts = user ? state.posts.filter((post) => post.user.handle === user.handle) : [];
...
postsElement.addEventListener('click', (event) => { const toggle = event.target.closest('.comment-toggle'); if (toggle) { const comments = document.querySelector(`#comments-${toggle.dataset.id}`); comments.classList.toggle('hidden'); return; } const button = event.target.closest('.rate-button'); if (!button || !requireLogin()) return; openRating(button.dataset.id); });
